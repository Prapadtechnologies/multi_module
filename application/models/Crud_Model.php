...
/*        $this->db->join('users u', 'from.id = epr.plan_id');*/
    ...
    /*function get_faqs_info_by_theme_type2($theme_type) {
    $this->db->select('faqs.id, faqs.answer, faqs.question, faqs.client_id, client.name, client.mail, client.theme_type as client_theme_type');
    $this->db->from('faqs');
    $this->db->join('client', 'client.id = faqs.client_id');
    $this->db->where('faqs.client_id', $theme_type);
    $query = $this->db->get();
    return $query->result_array();
    }*/
    ...
    $theme = $this->db->query("SELECT faqs.id, faqs.client_id, faqs.question, faqs.answer, client.name, client.theme_type 
                              FROM faqs 
                              LEFT JOIN client ON faqs.client_id = client.id 
                              WHERE faqs.theme_type = $theme_type AND client.row_status = 1")
                     ->result_array();
    return $theme;
    }
    ...
    /*public function get_client_info() {
        return $this->db->order_by('id', 'DESC')
                        ->get_where('client', array('row_status' => 1))
                        ->result_array();
    }*/
